...
The approved faction concepts (buildings, workers, resource nodes) live in
`assets/concepts/` with their generator scripts (`assets/render_*.py`) as the
geometry source of truth. **Before implementing any faction building, unit, or
effect, read `docs/faction-asset-kit.md`**: it has the exact palettes, mesh
recipes, animation specs, and the step-by-step wiring instructions.
