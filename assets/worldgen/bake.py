...
def write_scenario(path, name, entities):
    """Write the world-fitted scenario as a standard .map file (same format
    as the hand-written template; see docs/maps.md)."""
    lines = [
        "# Auto-generated by assets/worldgen/bake.py: the crossfire_basin",
        "# template re-fitted onto this world's viable ground. Edit the",
        "# template, not this file; re-baking overwrites it.",
        f"name {name}",
        "",
    ]
    for kind, owner, x, z in entities:
        if owner is None:
            lines.append(f"{kind} {round(x)} {round(z)}")
        else:
            lines.append(f"{kind} {owner} {round(x)} {round(z)}")
...
def main():
    ...
    for key in keys:
        ...
        grid.save(path)
        write_scenario(os.path.join(OUT, key + ".map"),
                       stats["name"], stats["scenario"])
