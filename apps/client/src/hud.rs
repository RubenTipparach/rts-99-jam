...
pub fn card_actions(game: &Game) -> Vec<(CardAction, &'static str, &'static str, i64, i64)> {
    ...
    if game.selected_hq().is_some() {
        ...
        // The faction's first-wave specialists.
        if astro {
            push(CardAction::Train(UnitKind::Pyromancer), "Pyromancer", "F");
            push(CardAction::Train(UnitKind::Stormcaller), "Stormcaller", "C");
        } else {
            push(CardAction::Train(UnitKind::Hound), "Hound", "F");
            push(CardAction::Train(UnitKind::Javelin), "Javelin", "C");
        }
    }
