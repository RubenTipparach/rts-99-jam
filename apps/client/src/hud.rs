...
const ICON_PNGS: [&[u8]; 28] = [
    ...
    include_bytes!("../../../assets/icons/pyromancer.png"),
    include_bytes!("../../../assets/icons/stormcaller.png"),
    include_bytes!("../../../assets/icons/hound.png"),
    include_bytes!("../../../assets/icons/javelin.png"),
    include_bytes!("../../../assets/icons/storm-ward.png"),
    include_bytes!("../../../assets/icons/bunker.png"),
    include_bytes!("../../../assets/icons/athenaeum.png"),
    include_bytes!("../../../assets/icons/crucible.png"),
    include_bytes!("../../../assets/icons/conservatory.png"),
    include_bytes!("../../../assets/icons/aerie.png"),
    include_bytes!("../../../assets/icons/ley-nexus.png"),
    include_bytes!("../../../assets/icons/machine-shop.png"),
    include_bytes!("../../../assets/icons/arsenal.png"),
    include_bytes!("../../../assets/icons/radar-array.png"),
    include_bytes!("../../../assets/icons/starport.png"),
    include_bytes!("../../../assets/icons/fusion-reactor.png"),
    include_bytes!("../../../assets/icons/drydock.png"),
    include_bytes!("../../../assets/icons/missile-silo.png"),
];

// The console skin panels (assets/hud, generated by assets/gen_hud_skin.py):
// one themed set per faction - Hollowmen gunmetal flight deck, Astromancer
// porcelain-and-gold lectern. Keys 100+ in the shared decode cache.
#[cfg(target_arch = "wasm32")]
const SKIN_HOLLOW: (usize, usize, usize) = (100, 101, 102);
#[cfg(target_arch = "wasm32")]
const SKIN_ASTRO: (usize, usize, usize) = (103, 104, 105);

/// `(left wing, right wing, centre strip)` skin keys for the player faction.
#[cfg(target_arch = "wasm32")]
fn skin_for(game: &Game) -> (usize, usize, usize) {
    if game.player_is_astromancer() {
        SKIN_ASTRO
    } else {
        SKIN_HOLLOW
    }
...
#[cfg(target_arch = "wasm32")]
fn png_bytes(key: usize) -> &'static [u8] {
    match key {
        100 => include_bytes!("../../../assets/hud/hollow_wing_left.png"),
        101 => include_bytes!("../../../assets/hud/hollow_wing_right.png"),
        102 => include_bytes!("../../../assets/hud/hollow_console_mid.png"),
        103 => include_bytes!("../../../assets/hud/astro_wing_left.png"),
        104 => include_bytes!("../../../assets/hud/astro_wing_right.png"),
        105 => include_bytes!("../../../assets/hud/astro_console_mid.png"),
        i => ICON_PNGS[i],
    }
}
...
fn icon_canvas(idx: usize) -> Option<web_sys::HtmlCanvasElement> {
    ...
    CACHE.with(|cache| {
        ...
        let img = image::load_from_memory(png_bytes(idx)).ok()?;
...
fn action_icon(game: &Game, action: CardAction) -> usize {
    ...
    match action {
        ...
        CardAction::Train(UnitKind::Pyromancer) => 10,
        CardAction::Train(UnitKind::Stormcaller) => 11,
        CardAction::Train(UnitKind::Hound) => 12,
        CardAction::Train(UnitKind::Javelin) => 13,
        CardAction::Build(BuildingKind::StormWard) => 14,
        CardAction::Build(BuildingKind::Bunker) => 15,
        CardAction::Build(BuildingKind::Athenaeum) => 16,
        CardAction::Build(BuildingKind::Crucible) => 17,
        CardAction::Build(BuildingKind::Conservatory) => 18,
        CardAction::Build(BuildingKind::Aerie) => 19,
        CardAction::Build(BuildingKind::LeyNexus) => 20,
        CardAction::Build(BuildingKind::MachineShop) => 21,
        CardAction::Build(BuildingKind::Arsenal) => 22,
        CardAction::Build(BuildingKind::RadarArray) => 23,
        CardAction::Build(BuildingKind::Starport) => 24,
        CardAction::Build(BuildingKind::FusionReactor) => 25,
        CardAction::Build(BuildingKind::Drydock) => 26,
        CardAction::Build(BuildingKind::MissileSilo) => 27,
    }
}
...
fn info_icon(kind: sim::Kind, astro: bool) -> usize {
    match kind {
        ...
        sim::Kind::Heavy => 9,
        sim::Kind::Pyromancer => 10,
        sim::Kind::Stormcaller => 11,
        sim::Kind::Hound => 12,
        sim::Kind::Javelin => 13,
        sim::Kind::StormWard => 14,
        sim::Kind::Bunker => 15,
        sim::Kind::Athenaeum => 16,
        sim::Kind::Crucible => 17,
        sim::Kind::Conservatory => 18,
        sim::Kind::Aerie => 19,
        sim::Kind::LeyNexus => 20,
        sim::Kind::MachineShop => 21,
        sim::Kind::Arsenal => 22,
        sim::Kind::RadarArray => 23,
        sim::Kind::Starport => 24,
        sim::Kind::FusionReactor => 25,
        sim::Kind::Drydock => 26,
        sim::Kind::MissileSilo => 27,
        _ => 8,
    }
}
...
pub fn card_actions(game: &Game) -> Vec<(CardAction, &'static str, &'static str, i64, i64)> {
    ...
    let astro = game.faction_of(0) == crate::game::Faction::Astromancer;
    match game.selected_producer_kind() {
        Some(sim::Kind::Hq) => push(CardAction::Train(UnitKind::Worker), "Worker", "T"),
        Some(sim::Kind::Barracks) => {
            push(CardAction::Train(UnitKind::Infantry), "Infantry", "T");
            push(CardAction::Train(UnitKind::Heavy), "Heavy", "H");
        }
        Some(sim::Kind::Athenaeum) => {
            push(CardAction::Train(UnitKind::Pyromancer), "Pyromancer", "T");
            push(CardAction::Train(UnitKind::Stormcaller), "Stormcaller", "H");
        }
        Some(sim::Kind::MachineShop) => {
            push(CardAction::Train(UnitKind::Hound), "Hound", "T");
            push(CardAction::Train(UnitKind::Javelin), "Javelin", "H");
        }
        _ => {}
    }
    if game.has_worker_selected() {
        ...
        if astro {
            push(
                CardAction::Build(BuildingKind::StormWard),
                "Storm-Ward",
                "X",
            );
            push(CardAction::Build(BuildingKind::Athenaeum), "Athenaeum", "Q");
            push(CardAction::Build(BuildingKind::Crucible), "Crucible", "E");
            push(
                CardAction::Build(BuildingKind::Conservatory),
                "Conservatory",
                "R",
            );
            push(CardAction::Build(BuildingKind::Aerie), "Aerie", "F");
            push(CardAction::Build(BuildingKind::LeyNexus), "Ley Nexus", "C");
        } else {
            push(CardAction::Build(BuildingKind::Bunker), "Bunker", "X");
            push(
                CardAction::Build(BuildingKind::MachineShop),
                "Machine Shop",
                "Q",
            );
            push(CardAction::Build(BuildingKind::Arsenal), "Arsenal", "E");
            push(
                CardAction::Build(BuildingKind::RadarArray),
                "Radar Array",
                "R",
            );
            push(CardAction::Build(BuildingKind::Starport), "Starport", "F");
            push(
                CardAction::Build(BuildingKind::FusionReactor),
                "Fusion Reactor",
                "C",
            );
            push(CardAction::Build(BuildingKind::Drydock), "Drydock", "Z");
            push(
                CardAction::Build(BuildingKind::MissileSilo),
                "Missile Silo",
                "M",
            );
        }
...
pub fn draw(
    ...
    ctx.close_path();
    ctx.fill();
    // Console skin: the player faction's procedural panel set
    // (assets/gen_hud_skin.py) stretched into each section. The fill above
    // stays as the fallback if a panel fails to decode.
    let skin = |key: usize, x: f64, y: f64, dw: f64, dh: f64| {
        if let Some(src) = icon_canvas(key) {
            let _ = ctx.draw_image_with_html_canvas_element_and_dw_and_dh(&src, x, y, dw, dh);
        }
    };
    let (skin_l, skin_r, skin_m) = skin_for(game);
    skin(skin_l, 0.0, hf - HUD_H, div_l, HUD_H);
    skin(skin_r, div_r, hf - HUD_H, wf - div_r, HUD_H);
    skin(skin_m, div_l, hf - MID_H, div_r - div_l, MID_H);
    ...
    if !actions.is_empty() {
        ...
        for (k, &(action, label, hotkey, ore, carbon)) in actions.iter().enumerate() {
            ...
            // Per-kind queue badge: how many of THIS unit are queued; the
            // front-of-queue kind's button also carries the progress bar.
            if let (CardAction::Train(k), Some((kinds, queued, frac))) = (action, prod) {
                let code = sim::unit_kind_code(k);
                let mine = kinds[..(queued as usize).min(kinds.len())]
                    .iter()
                    .filter(|&&q| q == code)
                    .count();
                if mine > 0 {
                    ctx.set_font("bold 11px monospace");
                    ctx.set_fill_style_str("#ffd36b");
                    let _ = ctx.fill_text(&format!("{mine}"), bx + 3.0, by + 13.0);
                }
                if queued > 0 && kinds[0] == code && frac > 0.0 {
